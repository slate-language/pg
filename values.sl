...
export decoded(oid, text)
    ...
    // **`numeric` stays TEXT and that is the one decision here worth defending.** It is an arbitrary
    // precision decimal -- which is what a money column is -- and slate's `real` is a double, so
    // decoding it would quietly round values the database went to some trouble to keep exact.
    // A program that wants arithmetic asks for `::float8` and says so.
    if oid == Numeric then return text
    ...
    if element != null then return asArray(text, element)
...
asArray(text, element)
    ...
    while i < len(text) && !done
        ...
        if quoted
            ...
            push(out, element_(piece, wasQuoted, element))
...
element_(piece, wasQuoted, element)
    if !wasQuoted && piece == "NULL" then return null

    if !wasQuoted && piece.startsWith("{") then return asArray(piece, element)

    decoded(element, piece)
